...
**🇧🇷 巴西葡语 与 🇲🇽 墨西哥西语 双站都需做本地化匹配**。归一化规则略有差异：

| | 葡语 BR（巴西） | 西语 ES（墨西哥） |
|---|---|---|
| **去重音字符** | `ç` → `c`；`ã â` → `a`；`ô õ` → `o` | `ñ` → `n`；`á é í ó ú` → 元音 |
| **单复数归一** | 一般去 `-s`/`-es`；**特殊** `-ões` `-ães` 去重音后 `-oes`/`-aes` → `ao`（pães/limões/mães → pão/limão/mão） | 统一去 `-s`/`-es`（规则简单） |

**模型应根据用户锁定的站点调用对应规则**：
- 用户发**葡语**或点名巴西站 → 用 🇧🇷 葡语 BR 归一化
- 用户发**西语**或点名墨西哥站 → 用 🇲🇽 西语 ES 归一化
